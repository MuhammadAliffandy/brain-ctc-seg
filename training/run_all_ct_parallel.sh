...
CUDA_VISIBLE_DEVICES=2 nohup python train_se2_by_dataset.py --dataset ct > "log_se2_ct_${DATE}.txt" 2>&1 &
...
CUDA_VISIBLE_DEVICES=7 nohup python train_comparison_models.py --model attention --dataset ct > "log_attention_ct_${DATE}.txt" 2>&1 &

echo "[GPU 7] Training TransUNet (CT)..."
CUDA_VISIBLE_DEVICES=7 nohup python train_comparison_models.py --model transunet --dataset ct > "log_transunet_ct_${DATE}.txt" 2>&1 &
