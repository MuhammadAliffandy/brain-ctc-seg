...
def main():
    CSV_PATH = os.path.join(os.path.dirname(__file__), "public_eval_metrics.csv")
    OUT_FILE = os.path.expanduser("~/Clara/brain-ctc-seg/training/Journal_Figures/ROC_Curve_Public_Dataset.png")
    
    if not os.path.exists(CSV_PATH):
        print(f"❌ Error: File {CSV_PATH} tidak ditemukan.")
        print(f"Pastikan Anda sudah menjalankan evaluate_public.py terlebih dahulu!")
        return
    ...
    plt.title(f'ROC Curve - Public Dataset (Zero-Shot Generalization)', fontsize=16, fontweight='bold', pad=20)
